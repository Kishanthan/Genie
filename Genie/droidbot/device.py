...
from .device_state import DeviceState, CurrentStateNoneException

DEFAULT_NUM = '1234567890'
DEFAULT_CONTENT = 'Hello world!'
GET_STATE_RETRY_LIMIT = 10
...
class Device(object):
    ...
    def __init__(self, device_serial=None, is_emulator=False, output_dir=None, app_package_name=None,
        ...
        # add the uiautomator mode
        self.uiautomator_mode = uiautomator_mode
        ...
        self.utg_abstraction_strategy = utg_abstraction_strategy
        self.view_context_str_backtrack_level = view_context_str_backtrack_level

        # This flag indicates whether we need to parse view tree (identify independent views)
        # When executing mutants, we do not need to parse view tree.
        from droidbot.input_policy import POLICY_PROPERTY_FUZZING, POLICY_FUZZ_GEN, POLICY_FUZZ_GEN_SEEDS, POLICY_FUZZ_GEN_MUTANTS
        self.enable_parse_view_tree = True if policy_name in [POLICY_PROPERTY_FUZZING,
                                                              POLICY_FUZZ_GEN,
                                                              POLICY_FUZZ_GEN_SEEDS,
                                                              POLICY_FUZZ_GEN_MUTANTS] else False
        ...
        # add a new adapter: uiautomator wrapper
        self.uiautomator_wrapper = UiautomatorWrapperConn(device=self)
        self.logcat = Logcat(device=self)
        # TODO temporarily comment this to avoid that multiple fuzzing threads may write this file
        ...
        self.adapters = {
            self.adb: True,
            self.telnet: False,
            self.droidbot_app: True,  # we could reuse droidbot_app to get views via accessibility service
            self.minicap: False,
            # enable the uiautomator wrapper
            self.uiautomator_wrapper: False,
            # If the parallel mode is enabled, we will connect to logcat when executing each mutant test
            self.logcat: True if enable_parallel_mode is False else False,
            # TODO temporarily comment this to avoid that multiple fuzzing threads may write this file
    ...
    def disconnect(self):
        ...
        if self.output_dir is not None:
            temp_dir = os.path.join(self.output_dir, self.serial + "-temp")
    ...
    def dump_coverage(self, remote_coverage_data_file_path, local_coverage_data_file_path):
        ...
        self.send_intent(intent)
        self.pull_file(remote_coverage_data_file_path, local_coverage_data_file_path)
        self.adb.shell("rm " + remote_coverage_data_file_path)
    ...
    def get_window_stack_of_current_activity(self, current_activity):
        """
        Get the window stack of current activity
        :param current_activity: the given activity
        :return: Dict
        """
        # data structure:
        #   key: window id
        ...
        lines = self.adb.shell("dumpsys window windows").splitlines()
        for line in lines:
            line = line.strip()
            if line.startswith("Window #"):
                window_id = line[7:]
            elif line.startswith("mAppToken="):
                if current_activity in line:
                    # record the window if the window belongs to the current activity
                    window_stack[window_id] = line[10:]
    ...
    def collect_granted_runtime_permissions(self):
        """
        get the runtime permissions of the app under test
        Note: the method must be called right after installing the app with -g
        :return: list, the list of granted runtime permissions
        """
        # get the lines after "runtime permissions"
        runtime_permission_cmd = "adb -s {} shell dumpsys package {}".format(self.serial, self.app_package_name)

        p = subprocess.run(runtime_permission_cmd.split(), stderr=subprocess.DEVNULL, stdout=subprocess.PIPE)
        out: str = p.stdout.decode()

        is_runtime_permission = False

        self.runtime_permissions = []
        for line in str.splitlines(out):
            if not is_runtime_permission:
                if 'runtime permissions' in line:
                    is_runtime_permission = True
                continue
            print(line)
            line = str.strip(line)
            line = line.split(":")[0]
            self.runtime_permissions.append(line)

    # get the runtime permissions
    def get_granted_runtime_permissions(self):
    ...
    def take_screenshot(self):
        ...
        # append tag with a unique number
        tag += '-' + str(Device.unique_screenshot_id)
        Device.unique_screenshot_id += 1

        local_image_dir = os.path.join(self.output_dir, self.serial + "-temp")
        ...
        if self.adapters[self.minicap] and self.minicap.last_screen:
            ...
            # We use /data/data/#{app_package_name} (not /sdcard) as the directory to temporarily store screenshots.
            #  This directory should be always readable and writable.
            #  This fix avoids the tricky problem:
            #   when some app needs to load some pics from a Gallery app, a lot of deleted screenshot pics are
            #       not cleaned (although removed). This affects the model construction strategy.
            #   In addition, we removed the Android Gallery app and only leave Android Photo app there.
            #       The pics in the Android Gallery app cannot be selected due to no views are clickable.
            remote_image_path = "/data/data/%s/screen_%s.png" % (self.app_package_name, tag)
    ...
    def get_current_state(self, output_dir):
        self.logger.debug("getting current device state...")
        current_state = None
        try:
            # We add a loop checking (bounded times) to ensure droidbot_app return consistent GUI layouts
            # If we receive inconsistent layouts, it will affect the precision of our whole algorithm!!!
            for _count in range(GET_STATE_RETRY_LIMIT):
                time.sleep(0.1)
                base_views = self.get_views()
                while base_views is None:
                    time.sleep(0.2)
                    base_views = self.get_views()
                time.sleep(0.3)
                new_views = self.get_views()
                if str(base_views) == str(new_views):
                    views = base_views
                    print(_count)
                    break
            else:
                self.logger.warning("Droibot XMLs are not consistent, give up the retry after {} times!".
                                    format(GET_STATE_RETRY_LIMIT))
                views = base_views

            if views is None:
                self.logger.warning("the views is None!!!")
                raise CurrentStateNoneException()
            ...
            window_stack = self.get_window_stack_of_current_activity(foreground_activity)
            ...
            current_state = DeviceState(self,
                                        ...
                                        utg_abstraction_strategy=self.utg_abstraction_strategy,
                                        enable_parse_view_tree=self.enable_parse_view_tree,
                                        view_context_str_backtrack_level=self.view_context_str_backtrack_level)
            current_state.save2dir(output_dir)
            # print("state str: %s" % current_state.state_str)
        ...
        if not current_state:
            self.logger.warning("Failed to get current state, the current state is None!")
            raise CurrentStateNoneException()
        return current_state

    def recover_device_state(self, state_json_file, screenshot_path=None, first_state=False, last_state=False):
        self.logger.debug("recover current device state...")
        device_state = None
        try:
            with open(state_json_file) as file:
                text = file.read()
                ...
                device_state = DeviceState(self,
                                           ...
                                           window_stack=state_data['window_stack'],
                                           app_package_name=self.app_package_name,
                                           screenshot_path=screenshot_path,
                                           json_state_path=state_json_file,
                                           tag=state_data['tag'],
                                           state_str=state_data['state_str'],
                                           structure_str=state_data['state_str_content_free'],
            ...
            self.logger.warning("exception in recover_device_state: %s" % e)
            import traceback
            traceback.print_exc()
        if not device_state:
            self.logger.warning("Failed to recover device state!")
        return device_state
    ...
    def clear_data_and_restore_permissions(self, package_name, permissions):
        """
        clear the app data (including user data and cache)
        :param package_name: the app package name
        :param permissions: the app permission list
        """
        self.adb.clear_app_data(package_name)
        if self.grant_perm:
            self.adb.grant_runtime_permissions(package_name, permissions)
    ...
    def get_views(self):
        ...
        # get views via the uiautomator wrapper
        if self.uiautomator_wrapper and self.adapters[self.uiautomator_wrapper]:
            views = self.uiautomator_wrapper.get_views()
            if views:
                return views
            else:
                self.logger.warning("Failed to get views using uiautomator wrapper")
